				...
				JY.Model.edit("auDiv", "新增", function() {
					if (JY.Validate.form("auForm")) {
						...
						JY.Ajax.doRequest("auForm", bonuspath
								+ '/backstage/scrap/add', null, function(data) {
							that.dialog("close");
...
function getbaseList(init) {
	...
	JY.Ajax
			.doRequest(
					"baseForm",
					bonuspath + '/backstage/scrap/findByPage',
					null,
					function(data) {
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td class='center hidden-480'>"
										+ (i + leng + 1) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.repairNum) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.machinesName) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.model) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.machinesNum) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.price) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.prices) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.collarProject) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.deviceNum) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.scrpReason) + "</td>";
								if(l.batchStatus == '' || l.batchStatus == null){
									html += "<td class='center'></td>";
								}else if(l.batchStatus == 2 || l.batchStatus =='2'){
									html += "<td class='center'><span class='label label-danger'>待报废</span></td>";
								}else{
									html += "<td class='center'></td>";
								}
...
$('#delBatchBtn').on('click', function(e) {
	...
	if (chks.length == 0) {
		...
		JY.Model.confirm("确认要删除选中的数据吗?", function() {
			JY.Ajax.doRequest(null, bonuspath + '/backstage/scrap/delBatch', {
				chks : chks.toString()
			}, function(data) {
				JY.Model.info(data.resMsg, function() {
					search();
...
function check(deviceNum) {
	cleanForm();
	JY.Ajax.doRequest(null, bonuspath + '/backstage/scrap/find', {
		deviceNum : deviceNum
	}, function(data) {
		setForm(data);
...
function setForm(data) {
	var l = data.obj;
	$("#auForm input[name$='scrapId']").val(l.scrapId);
	$("#auForm input[name$='scrapName']").val(JY.Object.notEmpty(l.scrapName));
...
function cleanForm() {
	...
	$("#auForm input[name$='scrapId']").val('0');// 上级资源
	$("#auForm input[name$='scrapName']").val('');
	...
	JY.Ajax.doRequest(null, bonuspath + '/backstage/scrap/find', {
		...
		JY.Model.edit("auDiv", "修改", function() {
			if (JY.Validate.form("auForm")) {
				var that = $(this);
				JY.Ajax.doRequest("auForm", bonuspath
						+ '/backstage/scrap/update', null, function(data) {
					that.dialog("close");
					JY.Model.info(data.resMsg, function() {
						search();
					});
...
function del(id) {
	JY.Model.confirm("确认删除吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/scrap/del', {
			scrapId : id
		}, function(data) {
			JY.Model.info(data.resMsg, function() {
				search();

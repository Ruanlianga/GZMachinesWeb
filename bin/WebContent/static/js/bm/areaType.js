				...
				JY.Model.edit("auDiv", "新增", function() {
					if (JY.Validate.form("auForm")) {
						...
						JY.Ajax.doRequest("auForm", bonuspath
								+ '/backstage/areaType/add', null, function(data) {
							that.dialog("close");
...
function getbaseList(init) {
	...
	JY.Ajax
			.doRequest(
					"baseForm",
					bonuspath + '/backstage/areaType/findByPage',
					null,
					function(data) {
...
$('#delBatchBtn').on('click', function(e) {
	...
	if (chks.length == 0) {
		...
		JY.Model.confirm("确认要删除选中的数据吗?", function() {
			JY.Ajax.doRequest(null, bonuspath + '/backstage/areaType/delBatch', {
				chks : chks.toString()
			}, function(data) {
				JY.Model.info(data.resMsg, function() {
					search();
...
function check(id) {
	cleanForm();
	JY.Ajax.doRequest(null, bonuspath + '/backstage/areaType/find', {
		id : id
	}, function(data) {
		setForm(data);
	...
	JY.Ajax.doRequest(null, bonuspath + '/backstage/areaType/find', {
		...
		JY.Model.edit("auDiv", "修改", function() {
			if (JY.Validate.form("auForm")) {
				var that = $(this);
				JY.Ajax.doRequest("auForm", bonuspath
						+ '/backstage/areaType/update', null, function(data) {
					that.dialog("close");
					JY.Model.info(data.resMsg, function() {
						search();
					});
...
function del(id) {
	JY.Model.confirm("确认删除吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/areaType/del', {
			id : id
		}, function(data) {
			JY.Model.info(data.resMsg, function() {
				search();

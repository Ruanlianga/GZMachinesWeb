				...
				JY.Model.edit("auDiv", "新增", function() {
					if (JY.Validate.form("auForm")) {
						...
						JY.Ajax.doRequest("auForm", bonuspath
								+ '/backstage/model/add', null, function(data) {
							that.dialog("close");
...
function loadOrgTree(){
	JY.Ajax.doRequest(null,bonuspath +'/backstage/machineType/findAll',null,function(data){
...
function emptyRole(){
	$("#types").prop("value","");
	$("#auForm input[name$='type']").prop("value","0");
}
...
function showRole() {
	if(preisShow){
		...
		var obj = $("#types");
		var offpos = $("#types").position();
		$("#typeContent").css({left:offpos.left+"px",top:offpos.top+obj.heigth+"px"}).slideDown("fast");	
		preisShow=true;
	}
}
function clickRole(e, treeId, treeNode) {
	...
	if(check){
		...
		$("#types").val(p);
		$("#auForm input[name$='type']").prop("value",n);
		hideRole();
	}
}
function hideRole(){
...
function getbaseList(init) {
	...
	JY.Ajax
			.doRequest(
					"baseForm",
					bonuspath + '/backstage/model/findByPage',
					null,
					function(data) {
...
$('#delBatchBtn').on('click', function(e) {
	...
	if (chks.length == 0) {
		...
		JY.Model.confirm("确认要删除选中的数据吗?", function() {
			JY.Ajax.doRequest(null, bonuspath + '/backstage/model/delBatch', {
				chks : chks.toString()
			}, function(data) {
				JY.Model.info(data.resMsg, function() {
					search();
...
function check(id) {
	cleanForm();
	findModel();
	JY.Ajax.doRequest(null, bonuspath + '/backstage/model/find', {
		id : id
	}, function(data) {
		setForm(data);
	...
	JY.Ajax.doRequest(null, bonuspath + '/backstage/model/find', {
		...
		JY.Model.edit("auDiv", "修改", function() {
			if (JY.Validate.form("auForm")) {
				var that = $(this);
				JY.Ajax.doRequest("auForm", bonuspath
						+ '/backstage/model/update', null, function(data) {
					that.dialog("close");
					JY.Model.info(data.resMsg, function() {
						search();
					});
...
function del(id) {
	JY.Model.confirm("确认删除吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/model/del', {
			id : id
		}, function(data) {
			JY.Model.info(data.resMsg, function() {
				search();
...
function findModel(){
	JY.Ajax.doRequest("auForm",bonuspath +'/backstage/machineType/findByPage',{},function(data){
		$("#type").html("");

				...
				JY.Model.edit("auDiv", "新增", function() {
					if (JY.Validate.form("auForm")) {
						...
						JY.Ajax.doRequest("auForm", bonuspath
								+ '/backstage/partfactory/add', null, function(data) {
							that.dialog("close");
...
function getbaseList(init) {
	...
	JY.Ajax
			.doRequest(
					"baseForm",
					bonuspath + '/backstage/partfactory/findByPage',
					null,
					function(data) {
...
/*								html += "<td class='center'>"+ JY.Object.notEmpty(l.isConsumables) + "</td>";*/
								html += rowFunction(l);
								html += "</tr>";
							}
...
$('#delBatchBtn').on('click', function(e) {
	...
	if (chks.length == 0) {
		...
		JY.Model.confirm("确认要删除选中的数据吗?", function() {
			JY.Ajax.doRequest(null, bonuspath + '/backstage/partfactory/delBatch', {
				chks : chks.toString()
			}, function(data) {
				JY.Model.info(data.resMsg, function() {
					search();
...
function check(l) {
  var l=JSON.parse(l);
	cleanForm();
	//JY.Ajax.doRequest(null, bonuspath + '/backstage/partsType/find', {
	//	partsTypeId : id
	//}, function(data) {
		setForm(l);
...
//	var l = data.obj;
	...
	$("#auForm input[name$='factory']").val(l.factory);
	$("#auForm input[name$='address']").val(l.address);
	$("#auForm input[name$='companyMan']").val(JY.Object.notEmpty(l.companyMan));
	$("#auForm input[name$='mainPerson']").val(l.mainPerson);
	$("#auForm input[name$='phone']").val(l.phone);
	$("#auForm input[name$='scopeBusiness']").val(l.scopeBusiness);
	$("#auForm input[name$='notes']").val(l.notes);
	$("#auForm input[name$='picurl']").val(l.picurl);
	...
	setForm(l);
		JY.Model.edit("auDiv", "修改", function() {
			if (JY.Validate.form("auForm")) { 
				var that = $(this);
				JY.Ajax.doRequest("auForm", bonuspath
						+ '/backstage/partfactory/update', null, function(data) {
					that.dialog("close");
					JY.Model.info(data.resMsg, function() {
						search();
					});
...
function del(id) {
	...
	JY.Model.confirm("确认删除吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/partfactory/del', {
			id : id
		}, function(data) {
			JY.Model.info(data.resMsg, function() {
				search();

...
//		alert(deviceModel);
		window.location.href=bonuspath+"/backstage/outstoragequery/export?startTime="+startTime+"&endTime="+endTime+"&machineName="+machineName+"&machineModel="+machineModel+"&keyWord="+keyWord;  
	});
});
...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/outstoragequery/outStorageCheckQuery',
					null,
					function(data) {
		          // alert("data="+JSON.stringify(data));
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.thisCheckNum) + "</td>";

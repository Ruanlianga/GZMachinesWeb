...
$(function() {
	...
	$('#resetBtn').on('click', function(e) {
		...
		$("#deviceType option[value='1']").attr("selected","true");
		init = 1;
		getbaseList(1);
	});
...
function getbaseList(index) {
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/machine/findByPage',null,
		function(data) {
			...
			if (results != null && results.length > 0) {
				...
				for (var i = 0; i < results.length; i++) {
					...
					html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.companyName) + "</td>";
							html += "<td style='vertical-align:middle;' class='center'><a href='javascript:void(0)' onclick = 'queryImg(\""+l.id+"\",1)'>查看</a></td>";
							html += "<td style='vertical-align:middle;' class='center'><a href='javascript:void(0)' onclick = 'filePreview(\""+l.id+"\",2)'>查看</a></td>";
							html += "<td style='vertical-align:middle;' class='center'><a href='javascript:void(0)' onclick = 'filePreview(\""+l.id+"\",3)'>查看</a></td>";
							html += "<td style='vertical-align:middle;' class='center'><a href='javascript:void(0)' onclick = 'fileView(\""+l.id+"\",\""+l.optUrl+"\",\""+l.optName+"\")'>查看</a></td>";
...
function check(id,typeId) {
	...
	layer.open({
	  ...
	  maxmin: true,
	  area: ['1000px', '550px'],
	  content: bonuspath+'/backstage/machine/openflowForm'
	});
}
function receive(deviceCode) {
	...
	layer.open({
	  ...
	  maxmin: true,
	  area: ['1300px', '550px'],
	  content: bonuspath+'/backstage/machine/machineLifeCycleFlow'
	});
}
...
//	});
	layer.open({
		  type: 2,
		  title:["操作手册",'background-color: #27A3D9;color:#fff'],
		  //shadeClose:true,
		  //shade:false,
		  maxmin: true,
		  area: ['800px', '600px'],
...
function queryOptImg(id,optUrl){//查看图片
	localStorage.setItem("id",id);//设备id
	localStorage.setItem("optUrl",optUrl);//设备id
	//iframe层-父子操作
//	layer.open({
...
//	  content: bonuspath+'/backstage/machine/queryOptImgPage'
//	});
	if(optUrl == null || optUrl ==""||optUrl=="null"){
		layer.msg("无数据")
	}else{
	window.open(bonuspath+optUrl);
	}
}	
//	window.open(bonuspath+optUrl);

	...
	JY={Object:{notNull:function(obj) {//判断某对象不为空..返回true 否则 false
		...
		Model:{//loading框
			...
			check:function(id,title,fn){
				$("#"+id).removeClass('hide').dialog({width: 600,resizable:false,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"查看")+"</h4></div>",title_html:true,buttons:[{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(fn) == 'function'){fn.call(this);}}}]});
			},
			
			check1:function(id,title,fn){
				$("#"+id).removeClass('hide').dialog({width: 400,resizable:false,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"查看")+"</h4></div>",title_html:true,buttons:[{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(fn) == 'function'){fn.call(this);}}}]});
			},
			checkImage:function(id,title,fn){
				$("#"+id).removeClass('hide').dialog({width: 340,height:455,resizable:false,modal:true,title:"<div class='widget-header'><h5 class='smaller'>"+(JY.Object.notNull(title)?title:"查看")+"</h5></div>",title_html:true});
			},
			
			edit:function(id,title,savefn,cancelfn){
				$("#"+id).removeClass('hide').dialog({width: 600,resizable:false,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"修改")+"</h4></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i>&nbsp;保存","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
			},			
			//新增方法1
			edit1:function(id,title,savefn,cancelfn){
				$("#"+id).removeClass('hide').dialog({width: 800,resizable:false,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"修改")+"</h4></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i>&nbsp;保存","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
			},	
			//弹窗新增方法3
			edit3:function(id,title,savefn,cancelfn){
				$("#"+id).removeClass('hide').dialog({width: 400,resizable:false,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"修改")+"</h4></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i>&nbsp;保存","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
			},		
			//审核
			audit:function(id,title,savefn,rejectfn,cancelfn){
				$("#"+id).removeClass('hide').dialog({width: 800,resizable:false,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"审核")+"</h4></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i>&nbsp;同意","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html: "<i class='icon-undo bigger-110'></i>&nbsp;驳回","class":"btn btn-danger btn-xs",click:function(){if(typeof(rejectfn) == 'function'){rejectfn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
			},
			//调整
		...
		Validate:{//判断是否是英文数字,是返回true，不是返回false
				  ...
				  isPhone:function(str){if(/^1[3|5|8][0-9]\d{4,8}$/.test(str))return true;return false;},
				  ...
				  form:function(fromId,side) {var res=true;side=JY.Object.notNull(side)?side:1;
				  	$('#'+fromId+" input[jyValidate]").each(function(){
				  		...
				  		var jyValidate = $(this).attr("jyValidate").split(",");$.each(jyValidate,function(n,v){
				  			...
				  			else if(v=='phone'){if(JY.Object.notNull(that.val())){if(!JY.Validate.isPhone(that.val())){that.tips({side:side,msg : "电话号码不正确！",bg :'#FF2D2D',time:1});that.focus();res=false;}}}
				  		...
				  		return res;},
					newForm:function(fromId,side) {
			  			var obj = {};
					  	var res=true;
					  	side=JY.Object.notNull(side)?side:1;
					  	$('#'+fromId).find("[jyValidate]").each(function(){
				  		//if(res==false)return;
				  		var that=$(this);
				  		...
				  		$.each(jyValidate,function(n,v){
				  			...
				  			if(v=='required'){
				  				if(!JY.Object.notNull(that.val())){
				  					that.tips({side:side,msg : "必要字段！",bg:'#FF2D2D',time:1});
				  					res=false;
				  				}
				  			}else if(v=='email'){
				  				if(JY.Object.notNull(that.val())){
				  					if(!JY.Validate.isEmail(that.val())){
				  						that.tips({side:side,msg : "电子邮箱不正确！",bg :'#FF2D2D',time:1}
				  					);
				  						that.focus();
				  						res=false;
				  					}
				  				...
				  				if(JY.Object.notNull(that.val())){
				  					if(that.val() <= 0){
				  						that.tips({side:side,msg : "请输入正数!",bg :'#FF2D2D',time:1});
				  						that.focus();
				  						res=false;
				  					}
				  				}
				  				...
				  				if(JY.Object.notNull(that.val())){
				  					if(that.val() != 0){
				  						that.tips({side:side,msg : "请分配完剩余数!",bg :'#FF2D2D',time:1});
				  						that.focus();
				  						res=false;
				  					}
				  				}
				  			}else if(v=='pin'){//正整数pos-int-num 缩写
				  				if(JY.Object.notNull(that.val())){
				  					...
				  					if (!(number > 0 )){
				  						that.tips({side:side,msg : "请输入正整数！",bg :'#FF2D2D',time:1}
				  					);
				  					that.focus();res=false;}
				  				}
				  			}else if(v=='timeout'){
				  				if(JY.Object.notNull(that.val())){
			  						that.tips({side:side,msg : "时间越界！",bg :'#FF2D2D',time:1});
			  						res=false;
				  				}
				  			}else if(v=='phone'){
				  				if(JY.Object.notNull(that.val())){
				  					if(!JY.Validate.isPhone(that.val())){
				  						that.tips({side:side,msg : "电话号码不正确！",bg :'#FF2D2D',time:1}
				  					);
				  					that.focus();
				  					res=false;
				  					}
				  				...
				  				if(JY.Object.notNull(that.val())){
				  					if(!JY.Validate.isDate(that.val())){
				  						that.tips({side:side,msg : "日期格式不正确！",bg :'#FF2D2D',time:1}
				  					);
				  						that.focus();
				  						res=false;
				  						}
				  				}
				  			}else if(v=='datetime'){if(JY.Object.notNull(that.val())){
				  					if(!JY.Validate.isDatetime(that.val())){
				  						that.tips({side:side,msg : "日期时间格式不正确！",bg :'#FF2D2D',time:1});
				  						that.focus();
				  						res=false;
				  					}
				  				}
				  			...
				  			else if(v=='ch'){if(JY.Object.notNull(that.val())){if(!JY.Validate.isCH(that.val())){that.tips({side:side,msg:"只能输入中文汉字",bg :'#FF2D2D',time:1});that.focus();res=false;}}}
				  			//extend
				          });
				  		});
				  		return res;
		...
		Ajax:{//异步请求, form表单ID,url请求路径,param参数对象,如：{a:'test',b:2},fn回调函数
			doRequest:function(form,url,param,fn){var params = form || param || {};if (typeof form == 'string'){params = $.extend(param || {},JY.Object.serialize($("#" + form)),{menu:JY.Url.getParam("menu")});}$.ajax({type:'POST',url:url,
				data:params,dataType:'json',success:function(data, textStatus) { if(data.res==1){if (typeof(fn)=='function'){fn.call(this, data);}}else{if(JY.Object.notNull(data.resMsg))JY.Model.error(data.resMsg);}},error:function(){return;},beforeSend:function(){},complete:function(){}});},
 ...
 * @returns 
 */
function getObjectOfForm(formId){
	var id;
	var obj = {};
	$("#"+formId).find("[name]").each(function(){
		var param = $(this).attr("name");
 ...
 * @returns 
 */
function addOrDeleteOpt(that,o){
	var obj = {};
	var id;
	if(o.hasClass('yes')){
		id = $(that).find("[filed]").eq(0).val();
		delete object[id];
	}else{
 ...
 * @returns 
 */
function cutPageBeforeOption(){
	$(".singlecheckbox .yes:visible").each(function(){
		var obj = {};
		var id;
		$(this).parent().parent().find("[filed]").each(function(){
			var param = $(this).attr("filed").split(",");
			var value;
			if(param[1]=='val'){
				value = $(this).val();
 ...
 * @returns 
 */
function checkIconBindEvent(checkedFn,cancelFn){
	$(".singlecheckbox").find(".check_icon").on("click",function(event){
		var $a = $(event.target);
		var that = $a.parent().parent();
		addOrDeleteOpt(that,$a);
		$a.attr("hidden","hidden");
		$a.siblings().eq(0).removeAttr("hidden");
		
	});
	$(".singlecheckbox").find(".check_icon_mp").on("click",function(event){
		var $a = $(event.target);
		var that = $a.parent().parent();
		var id = $a.parent().prev().val();
		if($a.hasClass("yes")){//取消操作
			delete object[id];
			/*$(that).find(".input").val("");
	...
	$(".all_checkbox").find("img").on("click",function(event){
		var $a = $(event.target);
		$a.attr("hidden","hidden");
		$a.siblings().eq(0).removeAttr("hidden");
	});
 ...
 */
function setOn(that){
	var o = $(that).find(".singlecheckbox .check_icon:visible");
	o.trigger("click");
}
 ...
 * @returns 
 */
function setOnMp(that){
	var o = $(that).find(".singlecheckbox .check_icon_mp:visible");
	o.trigger("click");
}
 ...
 * @returns 
 */
function backShowChenkStatus(fn){
	$(".ids").each(function(){
		var id = $(this).val();
 ...
 * @author 无畏
 * @date 2019-06-28
 * @function 获得当前选择的id数组
 * @returns 
 */
function getIdArray(){
	var array = [];
	for(var property in object){
		array.push(object[property].id);
	}
	return array;
}
 ...
 * @returns 
 */
function getIdStringByObject(o){
	var idStr = "";
 ...
 * @returns 
 */
function getArrayByObj(o){
	var array = [];
	for(var property in o){
		array.push(o[property]);
	}
	return array;
}
 ...
 * @returns 
 */
function paramConversionToObjOfForm2(that,param){
	var id;
	var obj = param || {};
	$(that).find("[filed]").each(function(){
		var param1 = $(this).attr("filed").split(",");
		var value;
		if(param1[1]=='val'){
			value = $(this).val();
		}else if(param1[1]=='text'){
			value = $(this).text();
		}
		var param = param1[0];
...
function paramConversionToObjOfForm3(that){
	var id;
	var obj =  {};
	$(that).find("[filed]").each(function(){
		var param1 = $(this).attr("filed").split(",");
...
function validCheckboxCheckedNum(flag){
	...
	if(flag == 1){
		if($(".cb:checkbox:checked").size() > 1){
			var indexMsg = layer.confirm("<h4 style='color:red;'>该操作单次只能操作一条数据!</h4>", {btn: ['关闭']},function(){
				layer.close(indexMsg);
			});
			return false;
		}
...
function showMsgAndReflush(msg){
	var indexMsg = layer.confirm("<h4 style='color:red;'>"+msg+"</h4>", {btn: ['关闭']},function(){
		layer.close(indexMsg);
		window.getbaseList(); // 刷新父页面
	});
}

//数据导出增加token通用模板方法
function exportCommon(url, methods, params, fileName) {
	...
	xhr.open(methods, downLoadUrl, true)
	xhr.setRequestHeader("Authorization", token)
	xhr.responseType = 'blob'
	xhr.onload = function () {
		if (xhr.status === 200) {
			var blob = xhr.response;
			var filename = `${fileName}.xls`; // 下载文件的名称

...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/puttask/findByPage',
					{
		              ...
		              //alert("data="+JSON.stringify(data));
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								var l = results[i];
								console.log("venderName=", l.venderName);
								html += "<tr>";
								if(l.isSure==0 || l.isSure=='0'){
									var val = l.id +","+ l.taskId +","+ l.isSure +","+ l.maModelId +","+ l.prePutNum +","+ l.putPersonId;
								...
								/*html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.prePutNum) + "</td>";*/
								html += "<td style='vertical-align:middle;' class='center'><a href='#' onclick='viewBackCode("+ l.id +","+ l.supId +"," + l.maModelId +")'>" + JY.Object.notEmpty(l.prePutNum) + "</a></td>";
								var putPerson = l.putPerson;
								var isSure = l.isSure;
								if(putPerson == "" || putPerson == null){
									putPerson = "尚未指派";
								}
								html += "<td style='vertical-align:middle;cursor:pointer;' class='center hidden-480'><a href='#' onclick='edit("+ l.id +","+ l.taskId +","+ l.isSure +","+ l.maModelId +","+ l.prePutNum +","+ l.putPersonId +")'>" + putPerson + "</a></td>";
								if(isSure == "1"){
									html += "<td style='vertical-align:middle;' class='center'>已完成</td>"
								}else{
									html += "<td style='vertical-align:middle;' class='center'>未完成</td>"
								}
								html += rowFunction(l.id,l.taskId,l.maModelId,l.prePutNum,l.definitionName,l.isSure);
								html += "</tr>";
							}
...
function rowFunction(id,taskId,maModelId,prePutNum,definitionName,isSure) {
	var h = "";

	h += "<td style='vertical-align:middle;' class='center'>";
	h += "<div class='visible-md visible-lg hidden-sm hidden-xs btn-group'>";
	if(definitionName == "机具入库"){
		
	}else{
		if(isSure==0 || isSure=='0'){
			h += "<a href='#' title='删除' onclick='deleteReturn(&apos;" + id + "&apos;,&apos;" + taskId + "&apos;,&apos;" + maModelId + "&apos;,&apos;" + prePutNum + "&apos;)' class='aBtnNoTD' ><i class='icon-ban-circle color-red bigger-140'></i></a>";
		}
	}
    ...
    if(definitionName == "机具入库"){
		...
		if(isSure==0 || isSure=='0'){
			h += "<li><a href='#' title='删除' onclick='deleteReturn(&apos;" + id + "&apos;,&apos;" + taskId + "&apos;,&apos;" + maModelId + "&apos;,&apos;" + prePutNum + "&apos;)' class='aBtnNoTD' ><i class='icon-ban-circle color-red bigger-140'></i></a></li>";
		}
...
function deleteReturn(id,taskId,maModelId,prePutNum) {
	JY.Model.confirm("确认删除入库任务，回退维修吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/puttask/deleteReturn', {
			id:id,
			taskId:taskId,
			maModelId:maModelId,
			prePutNum:prePutNum
...
function isRelease(id,taskId,maModelId,prePutNum,putPersonId) {
	if(putPersonId == null || putPersonId == '' || putPersonId == 'null'){
		layer.msg('请指派客服代表！', {
			  offset: 't',
			  anim: 6
			});
		...
		JY.Model.confirm("确认发布吗？", function() {
			JY.Ajax.doRequest(null, bonuspath + '/backstage/puttask/isRelease', {
				id:id,
				taskId:taskId,
				maModelId:maModelId,
				prePutNum:prePutNum,
				putPersonId:putPersonId
			}, function(data) {
				JY.Model.info(data.resMsg, function() {
					search();
					getbaseList(1);
...
function viewBackCode(id,supId,maModelId){
	...
	layer.open({
		...
		maxmin: true,
		area: ['800px', '600px'],
		content:bonuspath + '/backstage/puttask/backcodelist'
	});
...
function edit(id,taskId,isSure,maModelId,prePutNum,putPersonId) {
	cleanForm();
...
function batchConfirmation(){
	...
	JY.Model.edit("auDiv", "指派客服代表", function() {
		...
		if (JY.Validate.form("auForm")) {
			var serviceId = $("#serviceId").val();
			JY.Ajax.doRequest(null, bonuspath+'/backstage/puttask/updatePutPersons', 
				{
					value:vals,
					putPersonId:serviceId,
				}, 

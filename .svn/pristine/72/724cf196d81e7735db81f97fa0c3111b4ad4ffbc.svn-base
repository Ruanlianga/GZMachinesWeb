...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/out/findByPage',
					{keyWord: keyWord},
					function(data) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td style='vertical-align:middle;' class='center'><label>" +
											"<input type='checkbox' name='checkId' class='ids' id='inp' value="+l.taskId+">" +
													"<input type='hidden' name='nextId' id='nextId' value="+l.maModelId+" ></label></td>";
								...
								if(l.isExamine==0 || l.isExamine=='0'){
									...
									html += "<td style='vertical-align:middle;' class='center'><span style='clolr:green;'>审核通过</span></td>";
								}else{
									html += "<td style='vertical-align:middle;' class='center'><span style='clolr:red;'>审核驳回</span></td>";
								}
								html += rowFunction(l.taskId,l.maModelId,l.isExamine,l.mTaskId);
								html += "</tr>";
							}
...
function batchAudit() {
	...
	$.ajax({
		type:'POST',
		url:bonuspath+'/backstage/out/batchAudit',
		async:true,
		data:data,
...
function rowFunction(taskId,maModelId,isExamine,mTaskId) {
	var h = "";
	h += "<td style='vertical-align:middle;' class='center'>";
	h += "<div class='visible-md visible-lg hidden-sm hidden-xs btn-group'>";
	if(isExamine == 1 || isExamine == '1'||isExamine == 2 || isExamine == '2'){
	}else {
		h += "<a href='#' title='通过' onclick='check(&apos;" + taskId + "&apos;,&apos;" + maModelId + "&apos;,&apos;" + mTaskId + "&apos;)' class='aBtnNoTD' ><i class='icon-ok color-p bigger-140'></i></a>";
...
function check(taskId,maModelId,mTaskId) {
	JY.Model.confirm("确认审核吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/out/isExamine', {
			taskId:taskId,
			maModelId:maModelId,
			mTaskId:mTaskId
		}, function(data) {
	...
	JY.Model.edit("auDiv", "审核驳回", function() {
		...
		if (JY.Validate.form("auForm")) {
			...
			JY.Model.confirm("确认审核驳回吗？", function() {
				JY.Ajax.doRequest(null, bonuspath + '/backstage/out/rejectExamine', {
					taskId:taskId,
					maModelId:maModelId,
					mTaskId:mTaskId,

...
public class BalanceAgreementModelCodeGroup {

	private Logger logger = Logger.getLogger(BalanceAgreementModelCodeGroup.class);

	private List<SettlementDetailsBean> collars;

	private List<SettlementDetailsBean> backs;

	public BalanceAgreementModelCodeGroup(List<SettlementDetailsBean> collars, List<SettlementDetailsBean> backs) {
		super();
		this.collars = collars;
		this.backs = backs;
	}

	public List<BalanceAgreementModelBean> groupByModelIdCode(){
		...
		for (SettlementDetailsBean collarBean : collars) {
			...
			String maCode = collarBean.getDeviceCode();
		...
		for (SettlementDetailsBean backBean : backs) {
			...
			String maCode = backBean.getDeviceCode();

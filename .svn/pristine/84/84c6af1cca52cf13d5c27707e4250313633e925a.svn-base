				...
				JY.Model.edit("auDiv", "新增", function() {
					if (JY.Validate.form("auForm")) {
						...
						JY.Ajax.doRequest("auForm", bonuspath
								+ '/backstage/partstype/add', null, function(data) {
							that.dialog("close");
...
function getbaseList(init) {
	...
	JY.Ajax
			.doRequest(
					"baseForm",
					bonuspath + '/backstage/partstype/findByPage',
					null,
					function(data) {
						...
						/*		html += "<td class='center'><label> <input type='checkbox' name='ids' value='"
								...
								function consumable(){
									if(l.isConsumable!=null&& l.isConsumable!=''){
										if(l.isConsumable=='1'){
											return "是";
										}else if(l.isConsumable=='0'){
										return "否";
										}
									}else{
										return " ";
								...
								html += "<td class='center hidden-480'>"
										+ (i + leng + 1) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.typeNum) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.typeName) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(l.remarks) + "</td>";
								html += "<td class='center'>"+ JY.Object.notEmpty(consumable(l.isConsumable)) + "</td>";
								html += rowFunction(l);
								html += "</tr>";
							}
...
//	var l = data.obj;
	...
	$("#auForm input[name$='typeNum']").val(l.typeNum);
	$("#auForm input[name$='typeName']").val(l.typeName);
	$("#auForm input[name$='remarks']").val(JY.Object.notEmpty(l.remarks));
	$("#auForm input[name$='isConsumable']").val(l.isConsumable);
}
	...
	setForm(l);
		JY.Model.edit("auDiv", "修改", function() {
			if (JY.Validate.form("auForm")) { 
				var that = $(this);
				JY.Ajax.doRequest("auForm", bonuspath
						+ '/backstage/partstype/update', null, function(data) {
					that.dialog("close");
					JY.Model.info(data.resMsg, function() {
						search();
					});
...
function del(id) {
	JY.Model.confirm("确认删除吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/partstype/del', {
			id : id
		}, function(data) {
			JY.Model.info(data.resMsg, function() {
				search();

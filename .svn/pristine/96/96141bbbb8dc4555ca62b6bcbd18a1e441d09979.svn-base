	...
	$('#addBtn').on('click',function(e) {
		...
		JY.Model.edit("auDiv", "新增", function() {
			if (JY.Validate.form("auForm")) {
				var that = $(this);
				JY.Ajax.doRequest("auForm", bonuspath+'/backstage/companyType/insert', null, function(data) {
					that.dialog("close");
...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest( "baseForm",bonuspath + '/backstage/companyType/findByPage',null,function(data) {
	...
	JY.Ajax.doRequest(null,bonuspath +'/backstage/role/findUserRole',{},function(data){
		...
		if(res != 1){
			...
			JY.Model.confirm("确认删除吗？",function(){	
				JY.Ajax.doRequest(null,bonuspath +'/backstage/companyType/delete',{id:id},function(data){
...
function edit(id){
	cleanForm();
		JY.Ajax.doRequest(null,bonuspath +'/backstage/companyType/find',{id:id},function(data){
			setForm(data);   
		    JY.Model.edit("auDiv","修改",function(){
		    	if(JY.Validate.form("auForm")){
					var that =$(this);
					//alert("ist="+ist);
					JY.Ajax.doRequest("auForm",bonuspath +'/backstage/companyType/update',{id:id},function(data){
					    that.dialog("close");
					    JY.Model.info(data.resMsg,function(){search();});	
					});
				}	

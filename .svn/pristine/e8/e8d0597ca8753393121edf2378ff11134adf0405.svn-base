				...
				JY.Model.edit("auDiv", "新增", function() {
					if (JY.Validate.form("auForm")) {
						...
						JY.Ajax.doRequest("auForm", bonuspath
								+ '/backstage/house/add', null, function(data) {
							that.dialog("close");
...
function getbaseList(init) {
	...
	JY.Ajax
			.doRequest(
					"baseForm",
					bonuspath + '/backstage/house/findByPage',
					null,
					function(data) {
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td class='center'>"+ JY.Object.notEmpty(l.resPerson) + "</td>";
...
$('#delBatchBtn').on('click', function(e) {
	...
	if (chks.length == 0) {
		...
		JY.Model.confirm("确认要删除选中的数据吗?", function() {
			JY.Ajax.doRequest(null, bonuspath + '/backstage/house/delBatch', {
				chks : chks.toString()
			}, function(data) {
				JY.Model.info(data.resMsg, function() {
					search();
...
function check(id) {
	cleanForm();
	findPerson();
	JY.Ajax.doRequest(null, bonuspath + '/backstage/house/find', {
		id : id
	}, function(data) {
		setForm(data);
...
function setForm(data) {
	...
	$("#auForm input[name$='resPerson']").val(JY.Object.notEmpty(l.resPerson));
...
function cleanForm() {
	...
	$("#auForm input[name$='resPerson']").val('');
	...
	JY.Ajax.doRequest(null, bonuspath + '/backstage/house/find', {
		...
		JY.Model.edit("auDiv", "修改", function() {
			if (JY.Validate.form("auForm")) {
				var that = $(this);
				JY.Ajax.doRequest("auForm", bonuspath
						+ '/backstage/house/update', null, function(data) {
					that.dialog("close");
					JY.Model.info(data.resMsg, function() {
						search();
					});
...
function del(id) {
	JY.Model.confirm("确认删除吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/house/del', {
			id : id
		}, function(data) {
			JY.Model.info(data.resMsg, function() {
				search();

...
//		alert(deviceName);
		var maModel = $("#maModel").val();
		var keyWord = $("#keyWord").val();
//		alert(deviceModel);
		window.location.href=bonuspath+"/backstage/outquery/export?startTime="+startTime+"&endTime="+endTime+"&maType="+maType+"&maModel="+maModel+"&keyWord="+keyWord;  
	});
});
...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/settlementDetails/getSettlementApply',
					{agreementId :agreementId,batch:batch},
					function(data) {
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								var leasePrice = l.leasePrice;
								if(leasePrice == "" || leasePrice == null){
									leasePrice = "尚未填写";
								}
								html += "<td style='vertical-align:middle;cursor:pointer;' class='center hidden-480'><a href='#' onclick='editLeasePrice("+ l.id +")'>" + leasePrice + "</a></td>";
...
function editStopDays() {
	var batch = $("#batch").val();
	var stopDays = $("#stopDays").val();
	JY.Ajax.doRequest("auForm", bonuspath + '/backstage/settlementDetails/editStopDays',
			{agreementId:agreementId,stopDays:stopDays,batch:batch}, function(data) {
				search();
			}
	);
}
function editLeasePrice(id) {
	JY.Model.edit("auDiv", "修改", function() {
		if (JY.Validate.form("auForm")) {
			var that = $(this);
			JY.Ajax.doRequest("auForm", bonuspath
					+ '/backstage/settlementDetails/editLeasePrice',{id:id}, function(data) {
				that.dialog("close");

	...
	$('#addBtn').on('click', function(e) {
		...
		JY.Model.edit("auDiv","新增",function(){
			 if(JY.Validate.form("auForm")){
				 var that =$(this);
				 JY.Ajax.doRequest("auForm",bonuspath +'/backstage/station/add',null,function(data){
				     that.dialog("close");      
...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath +'/backstage/station/findByPage',null,function(data){
...
$('#delBatchBtn').on('click', function(e) {
	...
	if(chks.length==0) {
		...
		JY.Model.confirm("确认要删除选中的数据吗?",function(){	
			JY.Ajax.doRequest(null,bonuspath +'/backstage/station/delBatch',{chks:chks.toString()},function(data){
				JY.Model.info(data.resMsg,function(){search();});
			});
		});		
	}		
...
function check(id){
	cleanForm();
		JY.Ajax.doRequest(null,bonuspath +'/backstage/station/find',{id:id},function(data){
		    setForm(data);
...
function setForm(data){
	...
	$("#auForm input[name$='description']").val(JY.Object.notEmpty(l.description));
...
function cleanForm(){
	...
	$("#auForm input[name$='description']").val('');
	hideRole();
}
	...
	cleanForm();
		JY.Ajax.doRequest(null,bonuspath +'/backstage/station/find',{id:id},function(data){
		    ...
		    JY.Model.edit("auDiv","修改",function(){
		    	if(JY.Validate.form("auForm")){
					var that =$(this);
					JY.Ajax.doRequest("auForm",bonuspath +'/backstage/station/update',null,function(data){
					    that.dialog("close");
					    JY.Model.info(data.resMsg,function(){search();});	
					});
				}	
...
function del(id){
	JY.Model.confirm("确认删除吗？",function(){	
		JY.Ajax.doRequest(null,bonuspath +'/backstage/station/del',{id:id},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
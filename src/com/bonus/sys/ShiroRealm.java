...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
        ...
        String usernameOrPhone = token.getUsername(); // 用户名或手机号
        char[] pwd = token.getPassword();
        String password = pwd != null ? new String(pwd) : "";

        UserBean u = userService.findUserBeanByLoginNameOrPhone(usernameOrPhone); // 通过登录名或手机号寻找用户

        if (u != null) {
            if (u.getSalt() != null) {
                // 组合username,两次迭代，对密码进行加密
                String pwdEncrypt = CipherHelper.createPwdEncrypt(u.getLoginName(), password, u.getSalt());
                AuthenticationInfo auth;
                if (u.getPasswd().equals(pwdEncrypt)) {
                    auth = new SimpleAuthenticationInfo(u.getLoginName(), password, getName());

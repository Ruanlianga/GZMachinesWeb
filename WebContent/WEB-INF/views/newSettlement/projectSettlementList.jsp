		...
		<c:choose>
			...
			<c:forEach items="${page.results}" var="slt" varStatus="index">
				<tr ondblclick="setChecked(${slt.id})">
					...
					<th class="center">${slt.company.name}</th>
					<th class="center">${slt.project.name}</th>
					<th class="center">${slt.baseMoney}</th>
					<th class="center">${slt.addMoney}</th>
					<th class="center">${slt.subMoney}</th>
					<th class="center">${slt.lastMoney}</th>
					<th class="center">${slt.sltMan}</th>
					<th class="center">${slt.sltDate}</th>
					<th  class="center  " >
						<i onclick="uploadFile(${slt.id})"   title="文件上传" class='icon-circle-arrow-up color-blue  bigger-140'></i> 

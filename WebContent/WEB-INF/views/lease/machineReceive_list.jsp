<%@ page contentType="text/html;charset=UTF-8" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ page import="com.bonus.core.DateTimeHelper"%>
<!DOCTYPE html >
<html lang="en">
<head>
<%@include file="../baseset.jsp" %>
<%@include file="../systemset.jsp" %>
<link rel="stylesheet" href="${bonuspath}/static/plugins/zTree/3.5/zTreeStyle.css" />
<link rel="stylesheet" href="${bonuspath}/static/js/theme/default/layer.css" />
<script src="${bonuspath}/static/plugins/zTree/3.5/jquery.ztree.core-3.5.min.js"></script>
<script type="text/javascript" src="${bonuspath}/static/plugins/jquery.jqprint-0.3.js"></script>
<script type="text/javascript" src="${bonuspath}/static/plugins/jQuery.print.js"></script>
<script src="${bonuspath}/static/js/layer.js"></script>
<link rel="stylesheet" href="${bonuspath}/static/js/layui/css/layui.css" />
<script src="${bonuspath}/static/js/layui/layui.js"></script>
<%
	String currentDate = DateTimeHelper.getNowDate();
%>
</head>
<style type="text/css">
	.filter-option {
		color: grey;
	}
	
	.btn-group>.btn, .btn-group+.btn {
		border-width: 1px;
	}
	
	.inputText{
		width: 220px;
		height: 34px;
	}
	
	.inputBtn{
		width: 100px;
		height: 35px;
		border:none;
		border-radius:3px;
		background-color: #4CD964;
		color: white;
	}
	.ui-draggable{
		width: 1600px !important;
	}
</style>
<body>
<div class="page-content">
		<div class="row-fluid">	
			<div class="col-xs-12">
					<form id="baseForm" class="form-inline" method="POST" onsubmit="return false;">
						<div class="row">
							<div class="widget-main">	
								<font>&emsp;&nbsp;申请日期：</font> 
								<div class="layui-input-inline">
						        	<input type="text" value="<%=currentDate%>" class="input-large" id="startTime" name="startTime" readonly/>
						        	<input type="text" value="<%=currentDate%>" class="input-large" id="endTime" name="endTime" readonly />
						    	</div>
						    	<input type="text" name="keyWord" placeholder="这里输入关键词" class="input-large">			
								<button id='searchBtn' class="btn btn-warning  btn-sm" title="过滤" type="button" onclick="getbaseList(1)">
									<i class="icon-search bigger-120 icon-only"></i>
								</button>
							</div>				
						</div>
						<input type='hidden' class='pageNum' name='pageNum' value='1'/>
						<input type='hidden' class='pageSize'  name='pageSize' value='10'/>
						<iframe name="downloadFrame" style="display: none;" frameborder="0"></iframe>
					</form>
					<table id="baseTable" class="table table-striped table-bordered table-hover" >
						<thead>
							<tr>
								<th style="width:4%"  class="center hidden-480">序号</th>
								<th style="width:10%" class="center">领料时间</th>
								<th style="width:10%" class="center">租赁单位</th>
								<th style="width:15%" class="center">工程名称</th>
								<th style="width:10%" class="center">领料单号</th>
								<th style="width:10%" class="center">协议号</th>
								<th style="width:7%" class="center">领料人</th>
								<th style="width:10%" class="center">联系方式</th>
								<!-- <th style="width:9%" class="center">审核情况</th> -->
								<th style="width:9%" class="center">批准情况</th>
								<th style="width:7%" class="center">操作</th>
							</tr>
						</thead>
						<tbody></tbody>
					</table>
					<div class="row">
						<div class="col-sm-2">
							<!-- <button class="btn btn-info btn-sm" onclick="exportData();">导出数据</button> -->
						<div class="dataTables_info customBtn" >
							<!-- 	&emsp;<a href="#" title="导出数据"  class="lrspace3" onclick="exportData();" ><i class='icon-download color bigger-220'></i></a>  -->
						</div>							
						</div>
						<div class="col-sm-10">
							<!--设置分页位置-->
							<div id="pageing" class="dataTables_paginate paging_bootstrap">
								<ul class="pagination"></ul>
							</div>
						</div>
					</div>
			<%@include file="machineReceive_form.jsp" %>	
			<!-- #dialog-confirm -->
			<%@include file="../dialog.jsp" %>	
			</div>
		</div>
	</div>
<script src="${bonuspath}/static/js/lease/machineReceive.js?v=1"></script>		
</body>
</html>
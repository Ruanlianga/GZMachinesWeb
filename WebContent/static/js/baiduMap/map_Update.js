...
function createMarker(id,lon, lat, dev,jNum,sNum,site) { // 创建地图覆盖物
	...
	cent += "<span style='color:#ffffff;font-size:16px;'>地址： "
		+ JY.Object.notEmpty(site) + "</span><br />";
	cent += "</div>";
...
// 定义树节点初始数据
var zNodes1 = [
	{ "id":0, "pId":-1,open:true,"name":"GPS设备",icon:bonuspath + "/static/css/sys/images/home.gif"},
	];
...
function getOrgTreeData(keyWord) {
	$.ajax({
		type : 'POST',
		url : bonuspath + '/backstage/position/getMainTree',
		data : {
			keyWord:keyWord
		},

	...
	$('#addBtn').on('click',
			function(e) {
				...
				JY.Model.edit("auDiv", "新增", function() {
					var phone = $("#phone").val();
					var scopeBusiness = $("#scopeBusiness").val();
					if(phone == "" || phone == null){
						layer.msg("请输入联系电话");
						return false;
					}
					if(scopeBusiness == "" || scopeBusiness == null){
						layer.msg("请输入主要经营范围");
						return false;
					}
					...
					function(data) {
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.companyMan) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.mainPerson) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.phone) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.scopeBusiness) + "</td>";
								if(l.picUrl == '' || l.picUrl == null){
									html += "<td style='vertical-align:middle;' class='center'><a onclick='updMachinesPic(&apos;"+ l.id + "&apos;)'>上传</a></td>";
								}else{
									var picUrl = l.picUrl.replaceAll(/\\/g,"@");
									var picName = l.picName;
									html += "<td style='vertical-align:middle;' class='center'><a onclick='readMachinesPic(&apos;"+ picUrl +"&apos;)'>查看</a>&emsp;<a onclick='updMachinesPic(&apos;"+ l.id + "&apos;)'>上传</a></td>";
								}
...
function setForm(data) {
	...
	$("#scopeBusiness").val(JY.Object.notEmpty(l.scopeBusiness));
	$("#remark").val(JY.Object.notEmpty(l.remark));
...
function updMachinesPic(id){
	...
	layer.open({
	  ...
	  maxmin: true,
	  area: ['800px', '430px'],
	  content: bonuspath+'/backstage/vender/machinesPic'
	});
}
...
function readMachinesPic(picUrl){
	...
	layer.open({
		...
		maxmin: true,
		area: ['800px', '430px'],
		content: bonuspath+'/backstage/vender/readMachinesPic'
	});
}

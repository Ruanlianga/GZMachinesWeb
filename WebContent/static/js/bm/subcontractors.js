	...
	$('#addBtn').on('click',function(e) {
		...
		JY.Model.edit("auDiv", "新增", function() {
			if (JY.Validate.form("auForm")) {
				...
				JY.Ajax.doRequest("auForm", bonuspath+'/backstage/subcontractors/insert', null, function(data) {
					that.dialog("close");
...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest( "baseForm",bonuspath + '/backstage/subcontractors/findByPage',null,function(data) {
		...
		if (results != null && results.length > 0) {
			...
			for (var i = 0; i < results.length; i++) {
				...
				html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.chargePerson) + "</td>";
...
function del(id) {
	JY.Model.confirm("确认删除吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/subcontractors/delete', {
			id : id
		}, function(data) {
			JY.Model.info(data.resMsg, function() {
				search();
...
function edit(id){
	cleanForm();
		JY.Ajax.doRequest(null,bonuspath +'/backstage/subcontractors/find',{id:id},function(data){
			setForm(data);   
		    JY.Model.edit("auDiv","修改",function(){
		    	if(JY.Validate.form("auForm")){
					...
					//alert("ist="+ist);
					JY.Ajax.doRequest("auForm",bonuspath +'/backstage/subcontractors/update',{id:id},function(data){
					    that.dialog("close");
...
function cleanForm(){
	...
	$("#auForm input[name$='chargePerson']").val('');
...
function setForm(data){
	...
	$("#auForm input[name$='chargePerson']").val(l.chargePerson);
	$("#auForm input[name$='remark']").val(l.remark);
}

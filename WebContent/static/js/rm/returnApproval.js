...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/audit/findByPage',
					{
						keyWord: keyWord
					},
					function(data) {
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								"<input type='hidden' value="+l.modelId+">" +
								"<input type='hidden' value="+l.checkerId+" ></label></td>";
								...
								html += rowFunction(l.id,l.taskId,l.modelId,l.maNum,l.isApproval,l.checkerId,l.agreementId);
								html += "</tr>";
...
function backShowChenkStatus(){
	...
	$('input[type=checkbox]:checked').each(function(){
		...
		if(taskId ===undefined){
			...
			var modelId = $(this).next().next().val();
			var checkerId = $(this).next().next().next().val();
			var allId = id+","+taskId+","+modelId+","+checkerId;
...
function batchAudit() {
	...
	$.ajax({
		type:'POST',
		url:bonuspath+'/backstage/audit/batchApproval',
		async:true,
		data:data,
...
function rowFunction(id,taskId,modelId,maNum,isApproval,checkerId,agreementId) {
	var h = "";
	...
	if(isApproval == 1 || isApproval == '1'|| isApproval == 2 || isApproval == '2'){
		...
		h += "<a href='#' title='批准' onclick='check(&apos;" + id + "&apos;,&apos;" + taskId + "&apos;,&apos;" + modelId + "&apos;,&apos;" + maNum + "&apos;,&apos;" + checkerId + "&apos;)' class='aBtnNoTD' ><i class='icon-ok color-p bigger-140'></i></a>";
		h += "<a href='#' title='驳回' onclick='cancelCheck(&apos;" + id + "&apos;,&apos;" + taskId + "&apos;,&apos;" + modelId + "&apos;,&apos;" + maNum + "&apos;,&apos;" + agreementId + "&apos;)' class='aBtnNoTD' ><i class='icon-ban-circle color-red bigger-140'></i></a>";
	}
//	h += "<a href='#' title='删除' onclick='del(&apos;" + id + "&apos;)' class='aBtnNoTD' ><i class='icon-remove-sign col bigger-140'></i></a>";
	...
	if(isApproval == 1 || isApproval == '1'){
		...
		h += "<a href='#' title='审核' onclick='check(&apos;" + id + "&apos;,&apos;" + taskId + "&apos;,&apos;" + modelId + "&apos;,&apos;" + maNum + "&apos;&apos;" + checkerId + "&apos;)' class='aBtnNoTD' ><i class='icon-ok color-p bigger-140'></i></a>";
		h += "<a href='#' title='驳回' onclick='cancelCheck(&apos;" + id + "&apos;,&apos;" + taskId + "&apos;,&apos;" + modelId + "&apos;,&apos;" + maNum + "&apos;&apos;" + agreementId + "&apos;)' class='aBtnNoTD' ><i class='icon-ban-circle color-red bigger-140'></i></a>";
	}
...
function check(id,taskId,modelId,maNum,checkerId) {
	JY.Model.confirm("确认批准吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/audit/isApproval', {
			taskId:taskId,
			modelId:modelId,
			maNum:maNum,
			id:id,
...
function cancelCheck(id,taskId,modelId,maNum,agreementId) {
	console.info("taskId"+taskId+"modelId"+modelId+"maNum"+maNum+"id"+id+"agreementId"+agreementId)
	JY.Model.edit("auDiv", "批准驳回", function() {
		var that = $(this);
		if (JY.Validate.form("auForm")) {
			var approvalRemark = $("#remarks").val();
		
			JY.Model.confirm("确认批准驳回吗？", function() {
				JY.Ajax.doRequest(null, bonuspath + '/backstage/audit/putCancelApproval', {
					taskId:taskId,
					modelId:modelId,
					maNum:maNum,
					id:id,

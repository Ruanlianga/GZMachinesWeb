...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/audit/findAuditByPage',
					{
						keyWord: keyWord
					},
					function(data) {
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += rowFunction(l.id,l.taskId,l.modelId,l.maNum,l.isExamine,l.checkerId,l.agreementId);
								html += "</tr>";
...
function batchAudit() {
	...
	$.ajax({
		type:'POST',
		url:bonuspath+'/backstage/audit/batchAudit',
		async:true,
		data:data,
...
function rowFunction(id,taskId,modelId,maNum,isExamine,checkerId,agreementId) {
	var h = "";
	h += "<td style='vertical-align:middle;' class='center'>";
	h += "<div class='visible-md visible-lg hidden-sm hidden-xs btn-group'>";
	if(isExamine == 1 || isExamine == '1' ||isExamine == 2 || isExamine == '2'){
...
function check(id,taskId,modelId,maNum,checkerId) {
	JY.Model.confirm("确认审核吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/audit/isExamine', {
			taskId:taskId,
			modelId:modelId,
			maNum:maNum,
			id:id,
...
function cancelCheck(id,taskId,modelId,maNum,agreementId) {
	...
	JY.Model.edit("auDiv", "审核驳回", function() {
		...
		if (JY.Validate.form("auForm")) {
			var auditRemark = $("#remarks").val();
			JY.Model.confirm("确认审核驳回吗？", function() {
				
				JY.Ajax.doRequest(null, bonuspath + '/backstage/audit/putCancelExamine', {
					taskId:taskId,
					modelId:modelId,
					maNum:maNum,
					id:id,

...
//	agreementInfo();
	...
	$('#export').on('click',function(e) {
		...
		var isCollect;
		if ($("#isCollectOption").val() == 1) {
			isCollect = 0;
		}
		window.location.href=bonuspath+"/backstage/machinequery/export?machineName="+machineName+"&machineModel="+machineModel+"&keyWord="+keyWord + "&isCollect="+isCollect;
	});
});

function exportData(){
	/*$("#baseForm").attr("onsubmit","return true;");
	$("#baseForm").attr("action",bonuspath +'/backstage/machinequery/export');
	$("#baseForm").attr("target","downloadFrame");//iframe的名字
	$("#baseForm").submit();*/
	var url = bonuspath + '/backstage/machinequery/export';
	let params = {
		"machineName":$("#machineName").val(),
		"machineModel":$("#machineModel").val(),
		"keyWord":$("#keyWord").val(),
		"isCollect":$("#isCollectOption").val() == 1 ? 0 : 1
	}
	exportCommon(url, 'POST', params, '机具库存分布表')
}
...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/machinequery/machineStatusQuery',
					null,
					function(data) {
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length - 1; i++) {
								...
								if(i == 0){
									...
									html += "<tr>";
									html += "<td style='vertical-align:middle;' class='center hidden-480'>" + (i) + "</td>";
									html += "<td colspan='3' style='vertical-align:middle;' class='center'>合计</td>";
									html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(ll.storageNum)) + "</td>";
									...
									html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(ll.checkNum)) + "</td>";
									html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(ll.waitInputNum)) + "</td>";
							
									html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(ll.scrapNum)) + "</td>";
									html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(ll.loseNum)) + "</td>";
									
									if(parseInt(ll.totalStorageNum) > 0 ){
										html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(ll.totalStorageNum)) + "</td>";
									}else{
										html += "<td style='vertical-align:middle;' class='center'>"+ 0 + "</td>";
									}
								...
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.pMachineName) + "</td>";
								...
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(l.storageNum)) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(l.leaseNum)) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(l.repairNum)) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(l.checkNum)) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(l.waitInputNum)) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(l.scrapNum)) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(l.loseNum)) + "</td>";
								if(parseInt(l.totalStorageNum) > 0 ){
									html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(parseInt(l.totalStorageNum)) + "</td>";
								}else{
									html += "<td style='vertical-align:middle;' class='center'>"+ 0 + "</td>";
								}
								//alert(l.isCount);
								if(l.isCount == 0){
									...
									html += "<td style='vertical-align:middle;' class='center'>否</td>";
								}
								//html += rowFunction(l.id);
								if(l.isCollect == 0){
									html += "<td style='vertical-align:middle;' class='center'>";
									html += "<button class='btn btn-xs btn-info' onclick='collectRemoveMachine(\"" + l.typeId + "\")'> 取消收藏</button>";
									html += "&nbsp;<button class='btn btn-xs btn-success' onclick='showMachineDetail(\"" + l.typeId + "\")'><i class='icon-eye-open'></i> 详情</button>";
									html += "</td>";
								}else if(l.isCollect == 1){
									html += "<td style='vertical-align:middle;' class='center'>";
									html += "<button class='btn btn-xs btn-info' onclick='collectAddMachine(\"" + l.typeId + "\")'><i class='icon-star'></i> 收藏</button>";
									html += "&nbsp;<button class='btn btn-xs btn-success' onclick='showMachineDetail(\"" + l.typeId + "\")'><i class='icon-eye-open'></i> 详情</button>";
									html += "</td>";
								}
								html += "</tr>";
...
function showMachineDetail(typeId) {
	layer.open({
		type: 2,
		title: ['机具详情', 'background-color: #27A3D9;color:#fff'],
		shadeClose: true,
		shade: false,
		maxmin: true,
		area: ['1000px', '600px'],
		content: bonuspath + '/backstage/machinequery/detailList?typeId=' + typeId
	});
}

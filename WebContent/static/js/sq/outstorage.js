...
//		alert(deviceModel);
		...
		var url = bonuspath +'/backstage/outquery/export';
		var params = {
			keyWord,
			startTime,
			endTime,
		...
		exportCommon(url, 'GET', params, `${params.startTime}-${params.endTime}出库详细表`)
	});
});
...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/outquery/outStorageQuery',
					null,
					function(data) {
		          // alert("data="+JSON.stringify(data));
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.thisOutNum) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.outPerson) + "</td>";

...
//		alert(deviceModel);
		//window.location.href=bonuspath+"/backstage/putquery/export?startTime="+startTime+"&endTime="+endTime+"&machineName="+machineName+"&machineModel="+machineModel+"&keyWord="+keyWord;
		var url = bonuspath +'/backstage/putquery/export';
		var params = {
			keyWord,
			startTime,
			endTime,
		...
		exportCommon(url, 'GET', params, `${params.startTime}-${params.endTime}入库详细表`)
	});
});
...
function getbaseList(init) {
	...
	$(".pageNum").val(1);
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",bonuspath + '/backstage/putquery/putInStorageQuery',
					null,
					function(data) {
		          // alert("data="+JSON.stringify(data));
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.thisPutNum) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.putPerson) + "</td>";
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.putTime) + "</td>";
//								alert(l.isCount);
								if(l.isCount == 0){

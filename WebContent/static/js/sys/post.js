				...
				JY.Model.edit("auDiv", "新增", function() {
					if (JY.Validate.form("auForm")) {
						...
						JY.Ajax.doRequest("auForm", bonuspath
								+ '/backstage/post/add', null, function(data) {
							that.dialog("close");
...
function getbaseList(init) {
	...
	JY.Ajax
			.doRequest(
					"baseForm",
					bonuspath + '/backstage/post/findByPage',
					null,
					function(data) {
						...
						if (results != null && results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td style='vertical-align:middle;' class='center'>"+ JY.Object.notEmpty(l.duty) + "</td>";
...
$('#delBatchBtn').on('click', function(e) {
	...
	if (chks.length == 0) {
		...
		JY.Model.confirm("确认要删除选中的数据吗?", function() {
			JY.Ajax.doRequest(null, bonuspath + '/backstage/post/delBatch', {
				chks : chks.toString()
			}, function(data) {
				JY.Model.info(data.resMsg, function() {
					search();
...
function check(id) {
	cleanForm();
	JY.Ajax.doRequest(null, bonuspath + '/backstage/post/find', {
		id : id
	}, function(data) {
		setForm(data);
...
function setForm(data) {
	...
	$("#auForm input[name$='duty']").val(JY.Object.notEmpty(l.duty));
	...
	JY.Ajax.doRequest(null, bonuspath + '/backstage/post/find', {
		...
		JY.Model.edit("auDiv", "修改", function() {
			if (JY.Validate.form("auForm")) {
				var that = $(this);
				JY.Ajax.doRequest("auForm", bonuspath
						+ '/backstage/post/update', null, function(data) {
					that.dialog("close");
					JY.Model.info(data.resMsg, function() {
						search();
					});
...
function del(id) {
	JY.Model.confirm("确认删除吗？", function() {
		JY.Ajax.doRequest(null, bonuspath + '/backstage/post/del', {
			id : id
		}, function(data) {
			JY.Model.info(data.resMsg, function() {
				search();

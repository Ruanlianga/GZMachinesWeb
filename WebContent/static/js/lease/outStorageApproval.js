							...
							for (var i = 0; i < results.length; i++) {
								...
								html += "<td style='vertical-align:middle;' class='center'><label>" +
											"<input type='checkbox' name='checkId' class='ids' id='inp' value="+l.taskId+">" +
													...
													"<input type='hidden' name='nextnextId' id='nextnextId' value="+l.phone+">" +
													"<input type='hidden' name='nextnextId' id='nextnextId' value="+l.leaseProjectId+">" +
													"<input type='hidden' name='nextnextId' id='nextnextId' value="+l.planOutId+">" +
									"</label></td>";
								...
								html += rowFunction(l.taskId,l.maModelId,l.isApproval,l.outNum,l.mTaskId,l.storageNum,l.applyNumber,l.phone,l.leaseProjectId,l.planOutId);
								html += "</tr>";
							}
...
function rowFunction(taskId,maModelId,isApproval,outNum,mTaskId,storageNum,applyNumber,phone,leaseProjectId,planOutId) {
	var h = "";
	h += "<td style='vertical-align:middle;' class='center'>";
	h += "<div class='visible-md visible-lg hidden-sm hidden-xs btn-group'>";
	if (isApproval == 1 || isApproval == '1'||isApproval == 2 || isApproval == '2') {
	}else{
		h += "<a href='#' title='通过' onclick='check(&apos;" + taskId + "&apos;,&apos;" + maModelId + "&apos;,&apos;" + outNum + "&apos;,&apos;" + mTaskId + "&apos;,&apos;" + storageNum + "&apos;,&apos;" + applyNumber + "&apos;,&apos;" + phone + "&apos;,&apos;" + leaseProjectId + "&apos;,&apos;" + planOutId + "&apos;)' class='aBtnNoTD' ><i class='icon-ok color-p bigger-140'></i></a>";
...
function backShowChenkStatus(){
	...
	$('input[type=checkbox]:checked').each(function(){
		...
		const leaseProjectId = $(this).next().next().next().next().next().next().next().next().val();
		const planOutId = $(this).next().next().next().next().next().next().next().next().next().val();
		console.log("taskId=",taskId);
		console.log("maModelId=",maModelId);
		console.log("outNum=",outNum);
		console.log("storageNum=",storageNum);
		console.log("leaseProjectId=",leaseProjectId);
		console.log("planOutId=",planOutId);

		if (storageNum != undefined) {
			if (parseInt(storageNum) >= parseInt(outNum)) {
				const allId = taskId + "," + maModelId + "," + outNum + "," + leaseProjectId + "," + planOutId;
...
function findMsgDataList() {
	var list = new Array();
	$('input[type=checkbox]:checked').each(function(){
		const applyNumber = $(this).next().next().next().next().next().next().val();
		const phone = $(this).next().next().next().next().next().next().next().val();
		let leaseProjectId = $(this).next().next().next().next().next().next().next().next().val();
		let planOutId = $(this).next().next().next().next().next().next().next().next().next().val();
		if(applyNumber != undefined){
			const str = applyNumber + "-" + phone + "-" + leaseProjectId + "-" + planOutId;
			list.push(str);
		}
...
function check(taskId,maModelId,outNum,mTaskId,storageNum,applyNumber,phone,leaseProjectId,planOutId) {
